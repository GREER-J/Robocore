...
def test_event_manager_max_queue():
    ...
    # Ensure no more than max events are added
    assert event_manager.n_events_in_active_queue == 2
...
def test_event_processing_order():
    ...
    results = []
    event_manager.add_event_to_active(
        Event(2, lambda: results.append('second'), 1, 1))
    event_manager.add_event_to_active(
        Event(1, lambda: results.append('first'), 1, 1))
...
def test_scheduled_events_processed_at_time():
    ...
    event_manager.add_scheduled_event(
        Event(initial_time+5, dummy_fun1.process, 1, 2))
    event_manager.add_scheduled_event(
        Event(initial_time+10, dummy_fun2.process, 1, 2))
